...
// ===== Bind mobile drawer search on every page =====
document.addEventListener('DOMContentLoaded', () => {
  const m = document.getElementById('searchInputMobile');
  if (m) {
    m.addEventListener('keypress', e => {
      if (e.key === 'Enter') doSearch(m.value);
    });
    // mobile drawer 🔍 button
    const btn = m.nextElementSibling;
    if (btn && btn.tagName === 'BUTTON') {
      btn.addEventListener('click', () => doSearch(m.value));
    }
  }
});
...
function cardHTML(p) {
  ...
  const stockTag = p.stock > 0
    ? `<p class="product-rating"><span style="color:var(--success)">●</span> Бэлэн: ${p.stock} ширхэг</p>`
    : `<p class="product-rating" style="color:var(--accent)">○ Захиалгаар</p>`;
...
async function renderHome() {
  ...
  // Dynamic category counts + brand strip on home
  renderCatCounts(all);
  renderBrandStrip(all);
}

function renderCatCounts(all) {
  const grid = document.getElementById('catGrid');
  if (!grid) return;
  grid.querySelectorAll('[data-cat]').forEach(card => {
    const cat = card.dataset.cat;
    const n = all.filter(p => p.cat === cat).length;
    const label = card.querySelector('.cat-count');
    if (label) label.textContent = n === 0 ? 'Удахгүй' : `${n} бүтээгдэхүүн`;
    card.style.opacity = n === 0 ? '0.55' : '1';
  });
}

function renderBrandStrip(all) {
  const root = document.getElementById('brandRow');
  if (!root) return;
  const brands = [...new Set(all.map(p => p.brand))].sort();
  root.innerHTML = brands.length
    ? brands.map(b => `<span>${b}</span>`).join('')
    : '<span style="opacity:.6">Албан ёсны борлуулагч</span>';
}
...
async function initListing() {
  ...
  const brandRoot = document.getElementById('brandFilters');
  if (brandRoot) {
    const brands = [...new Set(allProducts.map(p => p.brand))].sort();
    brandRoot.innerHTML = brands.length
      ? brands.map(b => `<label><input type="checkbox" class="brand" value="${b}"> ${b}</label>`).join('')
      : '<small style="color:var(--text-soft)">Брэнд алга</small>';
    brandRoot.querySelectorAll('.brand').forEach(el => el.addEventListener('change', applyFilters));
  }
  ...
  document.getElementById('pdpRoot').innerHTML = `
    ...
    <div class="pdp-info">
      ...
      <div class="pdp-rating">
        ${p.stock > 0
          ? `<span style="color:var(--success)">● Бэлэн (${p.stock} ширхэг)</span>`
          : `<span style="color:var(--accent)">○ Захиалгаар авна</span>`}
      </div>
      ...
      <div class="pdp-features">
        ...
        <div class="pdp-feature"><div class="pdp-feature-icon">🛡️</div><div><h5>6 сарын баталгаа</h5><p>Албан ёсны</p></div></div>
        <div class="pdp-feature"><div class="pdp-feature-icon">💳</div><div><h5>Хуваан төлбөр</h5><p>Simple, Sono, Lendmn</p></div></div>
        <div class="pdp-feature"><div class="pdp-feature-icon">📞</div><div><h5>8001-2722</h5><p>Утсаар лавлана уу</p></div></div>
      </div>
...
async function renderCart() {
  ...
  itemsRoot.innerHTML = cart.map(item => {
    ...
    if (!p) return '';
    const lineTotal = p.price ? p.price * item.qty : null;
    return `
      <div class="cart-item">
        ...
        <div class="cart-item-info">
          ...
          <span class="price">${lineTotal != null ? fmtPrice(lineTotal) : 'Утсаар лавлах'}</span>
        </div>
  ...
  document.getElementById('sumTotal').textContent = fmtPrice(subtotal);
}

function checkout() {
  const cart = getCart();
  if (!cart.length) { toast('Сагс хоосон байна'); return; }
  window.location.href = 'tel:80012722';
}
