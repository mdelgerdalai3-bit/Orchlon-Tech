...
const ADMIN_PWD_KEY = 'orchlon_admin_pwd_hash';
...
async function adminLogin() {
  ...
  let stored = getStoredPwdHash();
  if (!stored) {
    stored = await hashPwd('orchlon2026');
    localStorage.setItem(ADMIN_PWD_KEY, stored);
  }
...
async function changePassword() {
  const current = prompt('Одоогийн нууц үг:');
  if (current === null) return;
  const currentHash = await hashPwd(current);
  if (currentHash !== getStoredPwdHash()) { alert('Одоогийн нууц үг буруу байна'); return; }
  const newPwd = prompt('Шинэ нууц үг (хамгийн багадаа 6 тэмдэгт):');
  if (!newPwd || newPwd.length < 6) { alert('Нууц үг 6+ тэмдэгт байх ёстой'); return; }
  const confirm2 = prompt('Шинэ нууц үг давтан оруулна уу:');
  if (newPwd !== confirm2) { alert('Нууц үг таарахгүй байна'); return; }
  localStorage.setItem(ADMIN_PWD_KEY, await hashPwd(newPwd));
