...
async function refreshDashboard() {
  ...
  tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--text-soft)">⏳ Supabase-ээс ачаалж байна…</td></tr>`;
  try {
    const all = await loadProducts(true);
    renderProductTable(all);
    renderStats(all);
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--accent)">❌ Алдаа: ${escapeHtml(err.message)}</td></tr>`;
  }
}
...
function renderProductTable(all) {
  ...
  `).join('') : `<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--text-soft)">
